...
function createCompiler(filename,path,compilerName){
    // Create the compiler
    var compiler = new File(path+"compiler.bat");
    compiler.encoding = 'UTF-8';
    compiler.open('w');
    var line = "cd /d %~dp0 & cd equations/ & "+compilerName+" -halt-on-error " +filename+ ".tex\nif \%ERRORLEVEL\% NEQ 0 \(type nul \> " +filename+ "\_error\.txt\)";
    compiler.write(line);
    compiler.close();
    
    // We create now the vbs file as a silent version of the compiler
    var silentCompiler = new File(path+"hidecmd.vbs");
    silentCompiler.encoding = 'UTF-8';
    silentCompiler.open('w');
    var silentLine = "Set WshShell = CreateObject(\"WScript.Shell\")\ncurrentDir = Mid(WScript.ScriptFullName, 1, (Len(WScript.ScriptFullName) - Len(WScript.ScriptName)))\ncurrentDir = currentDir + \"compiler.bat\"\nWshShell.Run chr(34) + currentDir + Chr(34), 0\nSet WshShell = Nothing";
    silentCompiler.write(silentLine);
    silentCompiler.close();
    return;
}
...
function gatherLatex(latexCode,nameOfFile,fontsize,path,compilerName){
    ...
    var pdfLatexCompiler = new File(path+"hidecmd.vbs");
    pdfLatexCompiler.execute();
    pdfLatexCompiler.close();
...
function cleanFiles(filename,path,keepAux,keepLog,keepTex){
    ...
    if (silentCompiler.exists){
        silentCompiler.remove();
    }
