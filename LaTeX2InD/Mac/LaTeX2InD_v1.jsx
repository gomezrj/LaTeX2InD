...
function createCompiler(filename,path,compilerName){
    // Create the compiler
    var compiler = new File(path+"compiler.command");
    var appleScript = new File(path+"compile.applescript");
    compiler.encoding = 'UTF-8';
    compiler.open('w');
    appleScript.open('w');
    var line = "cd && cd "+path+"equations/ && "+compilerName+" -halt-on-error " +filename+ ".tex";
    var compileLine = "do shell script \"chmod 755 "+path+"compiler.command\"";
    compiler.write(line);
...
function gatherLatex(latexCode,nameOfFile,fontsize,path,compilerName){
    ...
    var pdfLatexCompiler = new File(path+"compiler.command");
    var appleScript = new File(path+"compile.applescript");
